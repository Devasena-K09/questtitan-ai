...
export default function Home() {
  ...
  const [messages, setMessages] = useState([{ role: 'assistant', content: "Welcome to QuestTitan AI! Track your progress and climb the leaderboard." }]);
  ...
  useEffect(() => {
    ...
    return () => {
      document.querySelectorAll('script[src="https://checkout.razorpay.com/v1/checkout.js"]').forEach(s => s.remove());
    };
  }, []);
  ...
  const courses: Course[] = [
    {
      ...
      free: true,
      lessons: [
        { title: "Lesson 1: Introduction to Python & Print Statements", content: "Python is a high-level, interpreted language known for its simplicity and readability.\n\n**Your First Program:**\n```python\nprint('Hello, QuestTitan AI!')\n```" },
        { title: "Lesson 2: Variables, Data Types & Basic Operations", content: "Variables store data. Python is dynamically typed.\n\n**Example:**\n```python\nname = 'Tom Jerry'\nage = 20\n```" },
          ...
          {selectedCourse ? (
            <div>
              ...
              {(() => {
                ...
                if (!lesson) return <p className="text-zinc-400">No lessons available yet. More content coming soon.</p>;
